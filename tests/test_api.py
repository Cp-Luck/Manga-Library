...
def _mock_google_books(monkeypatch, status_code=200, payload=None):
    ...
    monkeypatch.setattr(main_module.httpx, "AsyncClient", lambda: _FakeAsyncClient(response))


# --- POST /volumes (manual add) ---

def test_add_volume_manually(client):
    resp = client.post("/volumes", json={"series_title": "Chainsaw Man", "volume_number": 4})
...
def test_add_volume_manually_rejects_duplicate_isbn(client):
    ...
    resp = client.post("/volumes", json={"series_title": "Berserk (dup)", "isbn": "1111111111111"})
...
def test_delete_volume(client):
    add_resp = client.post("/volumes", json={"series_title": "Vinland Saga", "volume_number": 1})
...
def test_update_volume_moves_to_new_series_and_cleans_up_empty_old_series(client):
    add_resp = client.post("/volumes", json={"series_title": "Fairy Tail: Misfiled", "volume_number": 63})
    volume_id = add_resp.json()["id"]
...
def test_get_series_volumes(client):
    add_resp = client.post("/volumes", json={"series_title": "Attack on Titan", "volume_number": 1})
    series_id = None
...
def test_scan_isbn_already_owned_skips_google_books_lookup(client, monkeypatch):
    client.post("/volumes", json={"series_title": "One Piece", "volume_number": 105, "isbn": "9781974735306"})
...
def test_scan_isbn_new_volume_creates_it_from_google_books_metadata(client, monkeypatch):
    _mock_google_books(monkeypatch, status_code=200, payload={
        "items": [{"volumeInfo": {
...
def test_scan_isbn_google_books_error_returns_502(client, monkeypatch):
    _mock_google_books(monkeypatch, status_code=500, payload={"error": {"message": "Backend Error"}})
...
def test_upload_cover_for_existing_volume(client):
    add_resp = client.post("/volumes", json={"series_title": "Spy x Family", "volume_number": 1})
    volume_id = add_resp.json()["id"]
